# RouteGuard

**Ваш свободный интернет** — универсальная VPN-платформа для OPKG Entware (Keenetic роутеры)

![Version](https://img.shields.io/badge/version-0.1.0-blue.svg)
![License](https://img.shields.io/badge/license-MIT-green.svg)
![Platform](https://img.shields.io/badge/platform-Entware-orange.svg)

---

## 📖 О проекте

RouteGuard — это модульная VPN-платформа для роутеров с поддержкой:

- **WireGuard, AmneziaWG, VLESS, Hysteria2, Shadowsocks, Trojan**
- **Умная маршрутизация** (домены, GeoIP, CIDR)
- **DNS с блокировкой рекламы** (DoH/DoT, DNS rewrite)
- **Обход DPI** без полного VPN

### Особенности

| ✨ | |
|---|---|
| 🚀 | Быстрая установка одной командой `curl \| sh` |
| 🎨 | Современный Web UI (Light/Pro режимы) |
| 🛡️ | Безопасность: шифрование, GPG-подписи, rate limiting |
| ⚡ | Оптимизировано для embedded: <30MB RAM, <10% CPU |
| 🧩 | Модульная архитектура — включайте только нужное |
| 🔄 | Автообновления с rollback при ошибке |

---

## 📦 Быстрый старт

### Установка

#### Шаг 1: Обновление пакетов

```bash
opkg update
```

#### Шаг 2: Установка curl (если не установлен)

```bash
opkg install curl
```

Или через wget:
```bash
opkg install wget
```

#### Шаг 3: Установка RouteGuard

```bash
curl -sL https://github.com/alexandr-kuz/RouteGuard/releases/latest/download/install.sh | sh
```

Или через wget:
```bash
wget -qO- https://github.com/alexandr-kuz/RouteGuard/releases/latest/download/install.sh | sh
```

### Через OPKG

```bash
opkg update
opkg install https://github.com/alexandr-kuz/RouteGuard/releases/latest/download/routeguard.ipk
```

### После установки

```
╔════════════════════════════════════════════════════╗
║   RouteGuard успешно установлен!                  ║
╚════════════════════════════════════════════════════╝

  Web UI:  http://192.168.1.1:8080
  Токен:   your-api-token-here

  Управление:
    /opt/etc/init.d/S50rguard start|stop|restart|status
```

---

## 🎯 Возможности

### 1. VPN Менеджер

- ✅ Импорт из подписок (URL)
- ✅ Импорт из файлов конфигурации
- ✅ Ручное создание профилей
- ✅ Экспорт/импорт конфигов
- ✅ Мониторинг статуса в реальном времени

**Поддерживаемые протоколы:**
- WireGuard / AmneziaWG
- VLESS (Reality, Vision)
- Hysteria2
- Shadowsocks
- Trojan

### 2. Маршрутизация трафика

- ✅ Доменная маршрутизация (`.ru` → direct, `.com` → VPN)
- ✅ GeoIP маршрутизация (по странам)
- ✅ CIDR правила
- ✅ Failover (резервирование)
- ✅ Балансировка нагрузки

### 3. DNS и блокировка рекламы

- ✅ DNS сервер на роутере
- ✅ DNS over HTTPS / TLS
- ✅ Кастомные DNS upstream
- ✅ DNS rewrite
- ✅ Блокировка рекламы (списки)
- ✅ Per-device политики

### 4. Обход DPI

- ✅ Селективный обход для доменов
- ✅ Фрагментация пакетов
- ✅ TLS манипуляции
- ✅ Низкое потребление CPU

---

## 🏗️ Архитектура

```
┌─────────────────────────────────────────────────────────┐
│                    Web UI (Vue 3)                       │
│              (Light Mode / Pro Mode)                    │
└─────────────────────────────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────┐
│                 Backend (Go + Gin)                      │
│                    REST API                             │
└─────────────────────────────────────────────────────────┘
        │                   │                   │
        ▼                   ▼                   ▼
┌───────────────┐ ┌─────────────────┐ ┌─────────────────┐
│  VPN Module   │ │ Routing Module  │ │  DNS Module     │
│  (sing-box)   │ │  (Xray-core)    │ │  (smartdns)     │
└───────────────┘ └─────────────────┘ └─────────────────┘
        │
        ▼
┌───────────────┐
│  DPI Module   │
│  (ByeDPI)     │
└───────────────┘
```

---

## 🛠️ Технологический стек

| Компонент | Технология |
|-----------|------------|
| Backend | Go 1.21 + Gin |
| Frontend | Vue 3 + Vite + Naive UI |
| VPN Core | sing-box |
| Routing | Xray-core |
| DNS | smartdns / AdGuard Home |
| DPI Bypass | ByeDPI / Zapret |

---

## 📚 Документация

- [📦 Установка](docs/installation.md)
- [⚙️ Конфигурация](docs/configuration.md)
- [🔌 VPN модуль](docs/modules/vpn.md)
- [🛣️ Маршрутизация](docs/modules/routing.md)
- [🌐 DNS модуль](docs/modules/dns.md)
- [🚫 DPI обход](docs/modules/dpi.md)
- [🖥️ API документация](docs/api.md)
- [💻 CLI команды](docs/cli.md)

---

## 🔧 Управление

### CLI

```bash
# Статус
routeguard status

# Запуск / остановка
routeguard start
routeguard stop
routeguard restart

# Обновление
routeguard update

# Бэкап / восстановление
routeguard backup > backup.json
routeguard restore < backup.json

# Дебаг
routeguard logs --follow
```

### Сервис

```bash
/opt/etc/init.d/S50rguard start
/opt/etc/init.d/S50rguard stop
/opt/etc/init.d/S50rguard restart
/opt/etc/init.d/S50rguard status
```

---

## 🔒 Безопасность

- ✅ API Token (JWT) для аутентификации
- ✅ HTTPS (TLS 1.3) для Web UI
- ✅ Шифрование чувствительных данных (AES-256)
- ✅ GPG подписи для обновлений
- ✅ Rate limiting (100 запросов/мин)
- ✅ CORS (только локальные IP)

---

## 📊 Системные требования

| Требование | Значение |
|------------|----------|
| Платформа | Entware (Keenetic) |
| Архитектура | MIPS, ARM, x86_64 |
| RAM (мин) | 128 MB |
| RAM (рек) | 256 MB |
| Storage | 100 MB |

---

## 🤝 Поддержка

- 📖 [Документация](docs/)
- 🐛 [Сообщить об ошибке](https://github.com/alexandr-kuz/RouteGuard/issues)
- 💡 [Идеи и предложения](https://github.com/alexandr-kuz/RouteGuard/discussions)

---

## 📝 Лицензия

MIT License — см. [LICENSE](LICENSE)

---

## 🙏 Благодарности

- [sing-box](https://github.com/SagerNet/sing-box) — универсальное VPN-ядро
- [Xray-core](https://github.com/XTLS/Xray-core) — маршрутизация
- [ByeDPI](https://github.com/ValdikSS/ByeDPI) — обход DPI
- [AdGuard Home](https://github.com/AdguardTeam/AdGuardHome) — DNS и блокировка

---

**RouteGuard** © 2024. Сделано с ❤️ для свободного интернета.
