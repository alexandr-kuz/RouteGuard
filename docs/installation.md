# 📦 Установка RouteGuard

## Требования

| Требование | Значение |
|------------|----------|
| Платформа | Entware (Keenetic роутеры) |
| Архитектура | MIPS, ARM, x86_64 |
| RAM (минимум) | 128 MB |
| RAM (рекомендуется) | 256 MB |
| Свободное место | 100 MB |

---

## 🚀 Быстрая установка

### Способ 1: curl | sh (рекомендуется)

```bash
curl -sL https://github.com/username/routeguard/releases/latest/download/install.sh | sh
```

**Что делает скрипт:**

1. ✅ Проверяет окружение (Entware, архитектура, память)
2. ✅ Загружает бинарник для вашей архитектуры
3. ✅ Проверяет контрольную сумму (SHA256)
4. ✅ Устанавливает зависимости (sing-box, smartdns, ByeDPI)
5. ✅ Создаёт директории и конфигурацию
6. ✅ Регистрирует сервис
7. ✅ Запускает RouteGuard

### Способ 2: OPKG пакет

```bash
# Обновление списков пакетов
opkg update

# Загрузка и установка
wget https://github.com/username/routeguard/releases/latest/download/routeguard.ipk
opkg install routeguard.ipk

# Или напрямую из URL
opkg install https://github.com/username/routeguard/releases/latest/download/routeguard.ipk
```

---

## 📋 После установки

### 1. Доступ к Web UI

```
╔════════════════════════════════════════════════════╗
║   RouteGuard успешно установлен!                  ║
╚════════════════════════════════════════════════════╝

  🌐 Web UI:  http://192.168.1.1:8080
  🔑 Токен:   your-api-token-here
```

**Важно:** Сохраните API токен в безопасном месте!

### 2. Первый вход

1. Откройте браузер и перейдите по адресу `http://<IP-роутера>:8080`
2. Введите API токен из вывода установщика
3. Вы попадёте на главную панель

---

## 🎛️ Управление сервисом

### Через init.d скрипт

```bash
# Запуск
/opt/etc/init.d/S50rguard start

# Остановка
/opt/etc/init.d/S50rguard stop

# Перезапуск
/opt/etc/init.d/S50rguard restart

# Статус
/opt/etc/init.d/S50rguard status
```

### Через CLI

```bash
# Статус
routeguard status

# Запуск / остановка
routeguard start
routeguard stop
routeguard restart

# Логи в реальном времени
routeguard logs --follow

# Обновление
routeguard update

# Бэкап
routeguard backup > backup.json

# Восстановление
routeguard restore < backup.json
```

---

## 🔧 Конфигурация

### Расположение файлов

```
/opt/etc/routeguard/
├── config.json          # Основной конфиг
├── .api_token           # API токен (не делитесь!)
├── .local_ip            # Локальный IP роутера
├── profiles/            # VPN профили
├── rulesets/            # Правила маршрутизации
└── certs/               # SSL сертификаты
```

### Логи

```
/opt/var/log/routeguard/
├── routeguard.log       # Основной лог
└── routeguard.log.1     # Архив логов
```

### Данные

```
/opt/var/lib/routeguard/
├── geoip/               # GeoIP базы
├── geosite/             # Geosite базы
└── backups/             # Бэкапы конфигурации
```

---

## 📝 Редактирование конфигурации

### Через Web UI

1. Откройте Web UI
2. Перейдите в **Настройки**
3. Измените нужные параметры
4. Нажмите **Сохранить**

### Через файл

```bash
# Открыть конфиг
vi /opt/etc/routeguard/config.json

# После изменений перезапустить сервис
/opt/etc/init.d/S50rguard restart
```

### Пример config.json

```json
{
    "api": {
        "port": 8080,
        "token": "your-token",
        "cors": true
    },
    "vpn": {
        "enabled": true,
        "core": "sing-box",
        "auto_connect": false
    },
    "routing": {
        "enabled": true,
        "mode": "domain",
        "default_route": "direct"
    },
    "dns": {
        "enabled": true,
        "port": 53,
        "upstream": "tls://1.1.1.1"
    },
    "dpi": {
        "enabled": false
    }
}
```

---

## 🔄 Обновление

### Автоматическое

```bash
routeguard update
```

### Вручную

```bash
# Через install.sh с версией
RG_VERSION=v0.2.0 curl -sL https://github.com/username/routeguard/releases/download/v0.2.0/install.sh | sh
```

### Автообновления (опционально)

Включите в конфиге:

```json
{
    "update": {
        "auto_check": true,
        "check_interval": "24h",
        "auto_install": false
    }
}
```

---

## 🗑️ Удаление

### Через uninstall скрипт

```bash
curl -sL https://github.com/username/routeguard/releases/latest/download/uninstall.sh | sh
```

### Вручную

```bash
# Остановка сервиса
/opt/etc/init.d/S50rguard stop

# Удаление файлов
rm -f /opt/bin/routeguard
rm -f /opt/etc/init.d/S50rguard
rm -rf /opt/etc/routeguard
rm -rf /opt/var/log/routeguard
rm -rf /opt/var/lib/routeguard

# (Опционально) Удаление зависимостей
opkg remove sing-box smartdns byedpi
```

---

## ❓ Решение проблем

### Сервис не запускается

```bash
# Проверка логов
cat /opt/var/log/routeguard/routeguard.log

# Проверка порта
netstat -tlnp | grep 8080

# Проверка прав
ls -la /opt/bin/routeguard
```

### Не доступен Web UI

1. Проверьте что сервис запущен: `routeguard status`
2. Проверьте порт: `netstat -tlnp | grep 8080`
3. Проверьте firewall роутера
4. Попробуйте другой браузер

### Ошибка при установке

```bash
# Проверка Entware
opkg --version

# Проверка места
df -h /opt

# Проверка памяти
free -m

# Перезапуск opkg
opkg update
```

### Сброс конфигурации

```bash
# Бэкап текущей конфигурации
routeguard backup > backup.json

# Удаление конфигурации
rm /opt/etc/routeguard/config.json

# Перезапуск (создастся новая)
/opt/etc/init.d/S50rguard restart
```

---

## 📞 Поддержка

- 📖 [Документация](../README.md)
- 🐛 [Сообщить об ошибке](https://github.com/username/routeguard/issues)
- 💡 [Обсуждение](https://github.com/username/routeguard/discussions)

---

**Далее:** [Настройка VPN](modules/vpn.md) | [Маршрутизация](modules/routing.md) | [DNS](modules/dns.md)
